...
class _MyAppState extends State<MyApp> {
  // Initialize without calling unavailable setConfig API.
  // If the package exposes timeout/config options in a different way,
  // apply them where supported (e.g., in startTesting) per package docs.
  final internetSpeedTest = FlutterInternetSpeedTest();

  final TAG = 'SPEED_TEST';
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: const Text('FlutterInternetSpeedTest example')),
        body: Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              // ✅ Tampilan Ping & Jitter
              Column(
                    ...
                    await internetSpeedTest.startTesting(
                      ...
                      onCompleted: (TestResult download, TestResult upload) {
                        setState(() {
                          ...
                          log("$TAG - Download completed");
                          log(
                            "$TAG - Download Rate: ${download.transferRate} ${download.unit}",
                          );

                          // ✅ Ambil ping dari hasil download
                          //   _ping = download.ping;
                          //   _jitter = download.jitter;
                        });
                        setState(() {
                      ...
                      onProgress: (double percent, TestResult data) {
                        log("$TAG - Progress: $percent");
                        log(
                          "$TAG - Transfer Rate: ${data.transferRate} ${data.unit}",
                        );
                        setState(() {
                          _unitText = data.unit == SpeedUnit.kbps
                              ? 'Kbps'
                              : 'Mbps';
                          if (data.type == TestType.download) {
                            ...
                            // ✅ Update ping realtime saat download
                            // if (data.ping > 0) _ping = data.ping;
                            // if (data.jitter > 0) _jitter = data.jitter;
                          } else {
                      ...
                      onDefaultServerSelectionDone: (Client? client) {
                        log(
                          "$TAG - Default server selected: IP ${client?.ip} ISP (${client?.isp}) COUNTRY(${client?.location?.country}) ASN(${client?.asn}) CITY ${client?.location?.city} }",
                        );
                        setState(() {
